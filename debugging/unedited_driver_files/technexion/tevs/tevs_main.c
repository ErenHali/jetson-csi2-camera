...
static int tevs_power_get(struct tegracam_device *tc_dev)
{
	...
	dev_err(dev, "DEBUG: Now inside tevs_power_get()\n"); // <-- ADD THIS
...
static int tevs_setup(struct tevs *tevs)
{
	...
	dev_err(tevs->dev, "DEBUG: Now inside tevs_setup()\n"); // <-- ADD THIS
	...
	dev_err(tevs->dev, "DEBUG: tevs_setup() - Calling tevs_try_on()...\n"); // <-- ADD THIS
			...
			const struct i2c_device_id *id)
{
	...
	dev_err(dev, "DEBUG: Probe step 1 - About to call tegracam_device_register\n"); // <-- ADD THIS

	ret = tegracam_device_register(tc_dev);
	if (ret) {
		dev_err(dev, "DEBUG: Probe step 2 - tegra camera driver registration FAILED with error %d\n", ret); // <-- MODIFY THIS
		return ret;
	}

	dev_err(dev, "DEBUG: Probe step 3 - tegracam_device_register SUCCESS\n"); // <-- ADD THIS
