...
static int ar0234_power_off(struct device *dev)
{
	...
	dev_dbg(dev, "Turning off power (DISABLED FOR DEBUGGING)\n");
...
static int ar0234_identify_module(struct ar0234 *ar0234)
{
	...
	if (ret) {
		dev_err(&client->dev, "failed to read chip id %x\n",
			0x180a); // this was AR0234_CHIP_ID
		return ret;
	}

	if (val != 0x180a) { // this was AR0234_CHIP_ID
		dev_err(&client->dev, "chip id mismatch: read %04x, expected %04x\n",
			val, 0x180a); 	// this was AR0234_CHIP_ID
		return -EIO;
	}
...
MODULE_DESCRIPTION("ON Semiconductor AR0234 V4L2 sensor driver");
